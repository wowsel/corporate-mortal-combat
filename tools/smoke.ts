...
import { RAW_DIR } from './manifest';
import { generateAudio, generateImage } from './openrouter';

const key = requireKey();
const what = process.argv[2] ?? 'image';
mkdirSync(RAW_DIR, { recursive: true }); // на свежем клоне assets/raw/ может не быть
if (what === 'image') {
  ...
  writeFileSync(join(RAW_DIR, '_smoke.png'), r.png); console.log('image ok', r.png.length, 'bytes, cost', r.cost);
} else if (what === 'voice') {
  ...
  writeFileSync(join(RAW_DIR, `_smoke_voice.${r.format}`), r.audio); console.log('voice ok', r.audio.length, r.format, 'cost', r.cost);
} else {
  ...
  writeFileSync(join(RAW_DIR, `_smoke_music.${r.format}`), r.audio); console.log('music ok', r.audio.length, r.format, 'cost', r.cost);
}
