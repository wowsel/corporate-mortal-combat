...
- Шанг Цунг: терпение 210, атака 14, слабость `data`, иммунитет `blame`, `final: false`. Текст события перед боем содержит намёк на слабость (`/данн|цифр|график|отчёт/i`) и иммунитет (`/вин|стрелк|ответствен/i`) — это проверяет тест из этапа 3.
...
- [ ] **Step 2:** Бой: Шанг Цунг слева-направо смотрит на героя, позы меняются без прыжков, зелёное свечение планшета видно на тёмном фоне. «Сослаться на данные» — двойной урон, «Перевести стрелки» — серая цифра и реплика из `immune`. Спецприём — `dim`, красная вспышка и реплика про ваше лицо по центру. Победа за 6–8 ходов при слабости (терпение 210), поражение — стресс +25, повтор с `repeatText` обоих событий.
