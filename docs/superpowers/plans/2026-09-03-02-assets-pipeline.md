...
| `sp_hero_attack` | sprite | `img/sprites/hero_attack.webp` | 700×900 | «…still facing RIGHT at the same three-quarter angle. HERO lunging forward to the RIGHT, pointing decisively at a presentation slide with a laser pointer, other hand holding the laptop open, confident face. Same magenta background, same framing scale.» | reference + `pt_hero_neutral` + `sp_hero_idle` | `pt_hero_neutral`, `sp_hero_idle` |
| `sp_hero_hurt` | sprite | `img/sprites/hero_hurt.webp` | 700×900 | «…still facing RIGHT at the same three-quarter angle. HERO recoiling backward (his back moves to the LEFT), papers flying close to his body, glasses askew, wincing. Same magenta background, same framing scale.» | то же | `pt_hero_neutral`, `sp_hero_idle` |
| `sp_hero_win` | sprite | `img/sprites/hero_win.webp` | 700×900 | «…still facing RIGHT. HERO in a modest triumphant pose, fist slightly raised, holding a framed certificate close to his chest (frame blank, no readable text), relieved smile. Same magenta background, same framing scale.» | то же | `pt_hero_neutral`, `sp_hero_idle` |

`chroma: "#FF00FF"`, `flip: false`. Каждый id из `references` обязан быть и в `dependsOn` (тест манифеста). Если сгенерированный герой смотрит влево — поставить `flip: true` и пересобрать кроп без перегенерации.
