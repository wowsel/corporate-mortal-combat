...
Генерация ассетов идёт через OpenRouter (`tools/gen-assets.ts` + манифест `assets/manifest.ts`).
...
- **Где что лежит**: сырые сгенерированные файлы — в `assets/raw/` (не в git, `.gitignore`); готовые ассеты игры — в `public/assets/img/**` (webp) и `public/assets/audio/**` (mp3/wav), путь к ним в манифесте относителен `public/assets/`.
...
**Статус:** этап 2 из 8 — пайплайн ассетов готов, герой сгенерирован ($0.34); титул, концовки, музыка и диктор ждут утверждения сюжета; боссы — прямоугольники.
